...
public class WhaleScript1: MonoBehaviour {
	...
	void Start () {
		...
		minVal = new Vector3(-200, 0, -200);
		maxVal = new Vector3(200, 50, 200);
	...
	IEnumerator Float() { 
		...
		if (destination == testVector) {
			...
			destination = new Vector3(testVector.x + Random.Range (minVal.x, maxVal.x),
			                          testVector.y + Random.Range(minVal.y, maxVal.y), 
			                          testVector.z + Random.Range(minVal.z, maxVal.z));
